...
function createCard(blog) {
    ...
    const isLiked = blog.likes?.some(id => id.toString() === (currentUser.id || currentUser._id)?.toString());
...
function processFeed(blogs) {
  ...
  // Defensive filtering for authors
  const userBlogs = blogs.filter(b => {
      const authorId = (b.author?._id || b.author || "").toString();
  ...
  shuffleArray(otherBlogs);

  return [...userBlogs, ...otherBlogs];
}
...
async function renderNextBatch() {
  ...
  const nextBatch = filteredBlogs.slice(displayedCount, displayedCount + BATCH_SIZE);
...
function renderTrendingBlogs() {
  ...
  const trending = [...allBlogs].sort((a,b) => (b.likes?.length || 0) - (a.likes?.length || 0)).slice(0, 5);
...
function renderRecentBlogs() {
  ...
  const recent = [...allBlogs].sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5);
